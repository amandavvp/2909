  ...
  try {
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        charts: {
          byStatus: byStatus.map(s => ({ name: statusLabels[s.status] || s.status, value: s._count.id, color: statusColors[s.status] || "#999" })),
          byDepartment: byDeptRaw.map(d => ({
          ...
          byPeriod: Object.entries(byPeriod).map(([date, count]) => ({ date, count })),
          byNeighborhood: Object.entries(neighMap).map(([name, count]) => ({ name, count })).sort((a, b) => b.count - a.count).slice(0, 15),
          byService: bySvcRaw.map(s => ({ name: svcMap[s.serviceId] || "?", count: s._count.id })),
        },
      },
    });
  } catch (error) {
